...
| `QL-DATA-003` | active | 混合学习活动扩展 | `78dc99e0`, `8f25e27b` | 低 | 每章再增 5 个混合形态活动：题目、翻转卡、拖拽分类、排序和配对 |
...
### QL-DATA-003 — 无线传感网络混合学习活动扩展

- 状态：`active`
- 首次提交：`78dc99e0`；题型兼容修正：`8f25e27b`
- 最近验证上游基线：DeepTutor `897fce52`
- 路径：`scripts/add_wsn_mixed_activities.py`、`tests/book/test_wsn_mixed_activities.py`；运行数据仅作用于指定书籍 `bk_a86a9945fc`
- 冲突风险：低；新增课程专用迁移脚本与测试，不修改通用块类型或存储格式。
- 目的：针对参数滑杆形式过于单一的问题，为 12 个理论章节分别再增加 5 个学习活动；每章固定包含 1 个原生 quiz、1 组原生 flash cards、1 个拖拽分类、1 个拖拽排序和 1 个概念配对，总计新增 60 个活动。拖拽活动同时提供点击或上移/下移替代操作，兼顾触屏和键盘式操作。
- 放置规则：60 个活动继续复用经审阅的小节标题映射，每个活动紧随对应理论小节出现；任何章节、类型、slug 或锚点不完整均拒绝写入。迁移只移除 `blk_wsnmix_` 前缀的旧版本块，因此重复执行不会复制内容，也不会改写已有参数实验、题目、图示或正文。
- 数据边界：真实写入要求显式备份目录；页面 JSON 原子替换；全部活动记录 `QL-BE-003` 生成的原生 prompt 与 history context。HTML 活动不含外部脚本、样式或网络依赖。
- 验证：Ruff 与语法检查、3 项专项 pytest 通过。真实教材隔离副本连续迁移两次后仍保持每章 5 个新增活动，12 章各类型均为 12 个，总计 60 个；全书块 ID 唯一，迁移前原有块 ID 和顺序不变，36 个 HTML 活动全部通过事件、自包含与无滑杆检查。原生提示词 API 对新增 60 个活动逐一完成真实 HTTP 调用；登录浏览器逐章确认 5 个活动、3 个新增 iframe 与 4 个单选项，并实际操作五种形式取得预期中文反馈。
- 测试部署：2026-09-18 已写入测试站书籍 `bk_a86a9945fc`；扩展前页面备份位于 `/app/data/user/workspace/book/backups/book_bk_a86a9945fc-pre-mixed-20260918T103034Z`。运行镜像为 `sha256:ed40cb10167279b6a4d24b3adc62dc108e4f76704111d0c603344307df21a828`，回滚镜像标签为 `qlearn-test:rollback-pre-wsn-mixed-0d596813`。
- 回滚：恢复迁移前 12 个页面备份；脚本不会在应用启动时自动执行。新增数据块均使用现有 `quiz`、`flash_cards` 和 `interactive` schema，无需数据库或代码回滚。
- 长期保留下游的理由：活动内容、题目和小节编排属于用户课程资产，不适合提交 DeepTutor 上游；使用的块类型与渲染能力保持上游兼容。
