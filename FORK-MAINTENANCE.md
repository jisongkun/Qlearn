...
| `QL-DATA-003` | active | 混合学习活动扩展 | 待本次提交 | 低 | 每章再增 5 个混合形态活动：题目、翻转卡、拖拽分类、排序和配对 |
...
- 验证：Ruff 与语法检查通过；3 项专项 pytest 通过。真实教材隔离副本连续迁移两次后仍保持每章 5 个新增活动，12 章各类型均为 12 个，总计 60 个；全书块 ID 唯一，迁移前原有块 ID 和顺序不变，36 个 HTML 活动全部通过事件、自包含与无滑杆检查。
